...
import { Shield, Zap, Newspaper, Briefcase, ListChecks, Copy, Check, RefreshCw, Wand2, CalendarPlus, Trash2, TrendingUp, AlertTriangle } from 'lucide-react'
...
function GenerateTab() {
  ...
  const allNews = [...GHANA_NEWS, ...GLOBAL_NEWS]
  ...
  const generate = async () => {
    ...
    const prompt = `${news ? `Real news hook: "${news.title}" (${news.meta}). Build the post around this story.` : extra || 'Use current Accra flooding June 2026 as the hook.'}
      ...
      <div className="card">
        <div className="field">
          ...
          <div className="pill-group">
            {GHANA_NEWS.map(n=><Pill key={n.id} selected={selNews===n.id} red onClick={()=>setSelNews(selNews===n.id?null:n.id)}><span style={{color:n.color,fontSize:9,fontWeight:600,marginRight:3}}>{n.badge}</span>{n.title.substring(0,42)}...</Pill>)}
            {GLOBAL_NEWS.map(n=><Pill key={n.id} selected={selNews===n.id} amber onClick={()=>setSelNews(selNews===n.id?null:n.id)}><span style={{color:n.color,fontSize:9,fontWeight:600,marginRight:3}}>{n.badge}</span>{n.title.substring(0,42)}...</Pill>)}
          </div>
        </div>
        <div className="field">
...
function NewsTab() {
  ...
  return (
    <div className="tab-content">
      <div className="card">
        <div className="section-label"><LiveDot/> Ghana — Breaking This Week</div>
        {GHANA_NEWS.map(n=>(
      ...
      <div className="info-box"><AlertTriangle size={13}/> News updated June 6 2026. Copy any hook and paste into the Generate tab extra context field.</div>
    </div>
  )
}
    ...
    <div className="app">
      <header className="header">
        <div className="header-inner">
          ...
          <div className="header-badge">Jun 2026</div>
        </div>
      </header>
      <nav className="nav">
